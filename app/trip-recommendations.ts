...
const dailySeeds: DailySeed[] = [
  {
    planId: "A", dayId: 1,
    吃: item("haikou-west-coast-food", "落地后的西海岸晚餐", "首晚不穿城追店，以酒店附近海南粉、清补凉或酒店简餐收尾。", ["航班正常就先吃清淡海南粉", "晚点超过 21:00 直接用酒店餐厅或外卖"], ["map-food-haikou-hengxingfa", "map-food-haikou-xitianmiao"], { price: "两人约 ¥80–180" }),
    穿: wear("海口", 1),
    住: item("haikou-marriott", "海口万豪 · 西海岸海景首晚", "用一晚海景阳台房完成落地缓冲，避开市中心和老城区折返。", ["预订时明确询问豪华海景大床/双床及楼层", "到店先确认阳台视野再放行李"], ["xhs-search-haikou-marriott", "hotel-haikou-marriott-overview", "map-hotel-haikou-marriott"], { price: "预算带 ¥700–1200/晚，非实时价格" }),
    行: item("route-wuhan-haikou", "武汉飞海口，再取车去西海岸", "武汉天河机场至海口美兰机场，落地取车后约 50–70 分钟到西海岸。", ["航班落地后预留 60–90 分钟取行李与取车", "导航先设酒店停车场，不追加骑楼老街"], ["transport-wuhan-airport", "transport-haikou-airport", "transport-haikou-parking"], { fallback: "晚点时直接入住，取消日落。" }),
    玩: item("haikou-west-coast", "只看一场西海岸日落", "首日唯一活动是酒店海岸散步，让身体先适应海南湿热。", ["日落前 40 分钟下楼即可", "只走酒店周边 30–45 分钟，不开车二次出门"], ["play-map-west-coast", "xhs-69c4017c0000000023021eb9"], { fallback: "下雨就在酒店泳池或大堂休息。" }),
  },
  {
    planId: "A", dayId: 2,
    ...
    穿: wear("万宁", 2),
    住: item("grand-hyatt-wanning", "万宁神州半岛君悦 · 连住两晚", "把万宁真正作为基地，不在石梅湾、兴隆和神州半岛之间反复搬行李。", ["确认预订的是明确标注海景的房型", "连住订单备注安静楼层、远离电梯"], ["xhs-6a33dac0000000002100add2", "hotel-grand-hyatt-wanning", "map-hotel-grand-hyatt-wanning"], { price: "预算带 ¥1000–1800/晚，非实时价格" }),
    行: item("route-haikou-wanning", "海口西海岸 → 神州半岛", "自驾约 2.5 小时，包含一次服务区休息；午后只办理入住。", ["10:00 前离店，避开海口出城高峰", "中途服务区休息 15–20 分钟，不临时加景点"], ["transport-haikou-wanning", "xhs-69c4017c0000000023021eb9"], { fallback: "暴雨时降低车速并把海岸活动全部删除。" }),
    玩: item("shenzhou-peninsula", "神州半岛傍晚短散步", "抵达后只在酒店海岸走一小段，不把换酒店日变成景点日。", ["日落前后选择 30–60 分钟散步", "海况红旗时只看海，不下水"], ["play-map-shenzhou", "xhs-69abee14000000000e03ce43"], { fallback: "雷暴时留在酒店。" }),
  },
  {
    planId: "A", dayId: 3,
    ...
    穿: wear("万宁", 3),
    住: item("grand-hyatt-wanning", "君悦第二晚 · 不换酒店", "石梅湾与兴隆当天往返神州半岛，晚间回同一间房彻底休息。", ["上午出发前让房务补水", "傍晚回店后不再安排夜市"], ["xhs-6a33dac0000000002100add2", "hotel-grand-hyatt-wanning", "map-hotel-grand-hyatt-wanning"], { price: "已计入连住预算" }),
    行: item("route-shimei-xinglong", "神州半岛 → 石梅湾 → 兴隆 → 神州半岛", "当天是万宁内部小环线，分段约 20–45 分钟，避免地图缩得像跨城。", ["上午先去石梅湾，午间转兴隆", "15:30 前返程，给酒店泳池和日落留时间"], ["transport-shimei-xinglong", "xhs-6a2b733d000000000802544e"], { fallback: "阵雨时直接从兴隆返回酒店。" }),
  ...
  {
    planId: "A", dayId: 4,
    吃: item("xincun-port-food", "新村港午餐，清水湾晚餐", "港口午餐以新鲜、清淡为先；入住后在英迪格观海餐厅解决晚餐。", ["渔排海鲜先确认单价、加工费和总重", "晕船或肠胃敏感就改吃陵水酸粉"], ["map-food-lingshui-xincun-seafood", "map-food-lingshui-yingjie", "xhs-6a79ad60000000002202c3da"], { price: "两人约 ¥180–380" }),
    穿: wear("陵水", 4),
    住: item("clearwater-indigo", "清水湾英迪格 · 连住三晚", "后三晚固定清水湾，海景、泳池和短距离海岸构成真正的慢住段。", ["优先确认海景房或临海房，不用套房堆预算", "到店确认停车、充电及早餐时段"], ["xhs-search-clearwater-indigo", "hotel-clearwater-indigo", "map-hotel-clearwater-indigo"], { price: "预算带 ¥1000–1800/晚，非实时价格" }),
  ...
  },
  {
    ...
    穿: wear("陵水", 5),
    住: item("clearwater-indigo", "英迪格第二晚 · 完整酒店日", "这一天的价值来自不挪窝：晨光海岸、午休、泳池和蓝调时刻。", ["上午先使用海岸，正午回房休息", "泳池时段与酒店活动表到店确认"], ["xhs-search-clearwater-indigo", "hotel-clearwater-indigo", "map-hotel-clearwater-indigo"], { price: "已计入连住预算" }),
    行: item("clearwater-local", "全天短距离步行", "不开长途车，只在酒店与清水湾海岸之间移动。", ["车留在酒店停车场", "海边往返带水和房卡，不带大量物品"], ["map-hotel-clearwater-indigo", "play-map-clearwater"], { fallback: "高温时把步行缩短为清晨与傍晚两段。" }),
  ...
  },
  {
    ...
    穿: wear("陵水", 6),
    住: item("clearwater-indigo", "英迪格第三晚 · 不搬行李", "自由日仍回清水湾，同一基地完成最后一晚。", ["提前整理次日机场用物", "晚间确认航班、油量和还车点"], ["xhs-search-clearwater-indigo", "hotel-clearwater-indigo", "map-hotel-clearwater-indigo"], { price: "已计入连住预算" }),
    行: item("route-clearwater-cdf", "清水湾 ⇄ 海棠湾免税城（可跳过）", "单程约 35–50 分钟，仅在有明确采购任务时往返。", ["按提货截止时间倒推离店", "购物结束立即返酒店，不再叠加后海或蜈支洲"], ["transport-clearwater-cdf", "transport-wanning-lingshui"], { fallback: "暴雨或无清单就留在酒店。" }),
  ...
  },
  {
    ...
    穿: wear("三亚", 7),
    住: item("departure-day-a", "不新增第四家酒店", "第七天直接由清水湾退房去三亚凤凰机场，保持全程三基地、只换两次酒店。", ["前一晚整理行李和易碎品", "退房时核对押金、充电器和证件"], ["hotel-clearwater-indigo", "transport-sanya-airport"]),
  ...
  },
  {
    ...
    穿: wear("海口", 1),
    住: item("haikou-west-coast-holiday", "海口西海岸假日 · 轻预算海景首晚", "用更低预算保留西海岸和海景诉求，但必须确认具体房型不是城市景。", ["预订页筛选明确写有海景的房型", "问清早餐、停车和取消规则"], ["xhs-search-haikou-west-coast-holiday", "hotel-haikou-west-coast-holiday", "map-hotel-haikou-west-coast-holiday"], { price: "预算带 ¥350–650/晚，非实时价格" }),
  ...
  },
  {
    ...
    穿: wear("万宁", 2),
    住: item("wanning-holiday-inn", "万宁神州半岛假日 · 连住两晚", "Plan B 用更可控预算住进神州半岛；海景餐厅不等于所有房间都有海景。", ["预订时逐字确认海景房型名称", "备注安静楼层并核对接驳与早餐"], ["xhs-search-wanning-holiday", "hotel-wanning-holiday", "map-hotel-wanning-holiday-inn"], { price: "预算带 ¥600–1000/晚，非实时价格" }),
  ...
  },
  {
    ...
    穿: wear("万宁", 3),
    住: item("wanning-holiday-inn", "神州半岛假日第二晚", "植物园和市场结束后回同一基地，不换酒店。", ["把湿衣物及时晾干", "晚间只使用酒店设施"], ["xhs-search-wanning-holiday", "hotel-wanning-holiday", "map-hotel-wanning-holiday-inn"], { price: "已计入连住预算" }),
  ...
  },
  {
    ...
    行: item("route-wanning-tufu", "万宁 → 土福湾", "直接南下约 1.5–2 小时，不经新村港，午后尽早入住。", ["退房后导航三正月酒店停车场", "途中只做一次短休，不增加景点"], ["transport-wanning-lingshui", "xhs-69c4017c0000000023021eb9"], { fallback: "暴雨时进一步放慢并取消海岸。" }),
...
export const hotelDecisionProfiles: HotelDecisionProfile[] = [
  { id: "haikou-marriott", name: "海口万豪酒店", planId: "A", base: 1, city: "海口", area: "西海岸", stay: "Day 1 · 1 晚", room: "优先豪华海景大床/双床；付款前核对房型名称与阳台", view: "官方客房页提供 180° 海景阳台房型", dining: "首晚可直接使用酒店餐饮，不进市区", parking: "自驾到酒店停车场；落地时先核对停车入口", fit: "远离市中心，适合落地缓冲和次日南下", cautions: ["只住一晚，不为套房过度加价", "机场至西海岸需预留 50–70 分钟"], priceBand: "¥700–1200/晚 · 九月预算带，非实时价格", evidenceIds: ["xhs-search-haikou-marriott", "hotel-haikou-marriott-overview", "hotel-haikou-marriott-rooms", "map-hotel-haikou-marriott"], verifiedAt: "2026-08-14" },
  { id: "grand-hyatt-wanning", name: "万宁神州半岛君悦酒店", planId: "A", base: 2, city: "万宁", area: "神州半岛", stay: "Day 2–3 · 2 晚", room: "预订页必须明确标注海景，不用园景房赌视野", view: "酒店位于神州半岛，房型含园景或海景", dining: "酒店内可完成换住日晚餐和第二晚休息", parking: "自驾入半岛，提前确认停车与充电", fit: "高预算放在真正连续两晚的万宁海岸基地", cautions: ["周末亲子客流可能增加", "不要把‘海边酒店’等同于‘所有房间海景’"], priceBand: "¥1000–1800/晚 · 九月预算带，非实时价格", evidenceIds: ["xhs-6a33dac0000000002100add2", "hotel-grand-hyatt-wanning", "map-hotel-grand-hyatt-wanning"], verifiedAt: "2026-08-14" },
  { id: "clearwater-indigo", name: "海南清水湾英迪格酒店", planId: "A", base: 3, city: "陵水", area: "清水湾", stay: "Day 4–6 · 3 晚", room: "优先明确海景房；泳池房与私人泳池套房只在价格合适时考虑", view: "官方页面区分海景、泳池通道与套房产品", dining: "酒店内观海餐厅适合完整酒店日", parking: "官方信息含停车与充电条件，到店复核", fit: "后三晚固定基地，最符合安静、海景和不奔波", cautions: ["入住通常 15:00、退房 12:00，以订单为准", "海况红旗时不下海"], priceBand: "¥1000–1800/晚 · 九月预算带，非实时价格", evidenceIds: ["xhs-search-clearwater-indigo", "hotel-clearwater-indigo", "map-hotel-clearwater-indigo"], verifiedAt: "2026-08-14" },
  { id: "haikou-west-coast-holiday", name: "海口西海岸假日酒店", planId: "B", base: 1, city: "海口", area: "西海岸", stay: "Day 1 · 1 晚", room: "筛选明确写有海景的房型，不以酒店临海推断房间视野", view: "官方介绍含 180° 全海景客房与空中泳池", dining: "首晚可在酒店完成餐饮", parking: "滨海西路自驾抵达，出发前核对停车政策", fit: "用轻预算保留海景与避开市中心", cautions: ["设施与房间维护状态需结合近期住客图复核", "晚航班直接入住"], priceBand: "¥350–650/晚 · 九月预算带，非实时价格", evidenceIds: ["xhs-search-haikou-west-coast-holiday", "hotel-haikou-west-coast-holiday", "map-hotel-haikou-west-coast-holiday"], verifiedAt: "2026-08-14" },
  { id: "wanning-holiday-inn", name: "万宁神州半岛假日度假酒店", planId: "B", base: 2, city: "万宁", area: "神州半岛", stay: "Day 2–3 · 2 晚", room: "确认具体海景房型；海景餐厅不代表基础房海景", view: "位于神州半岛，作为预算更稳的海岸基地", dining: "海景餐厅和周边茶坊可组合", parking: "官方位置距神州站约 15 分钟车程，自驾方便", fit: "保留万宁两晚，同时压低 Plan A 的酒店溢价", cautions: ["核对接驳、停车和早餐是否包含", "亲子度假属性可能带来时段性嘈杂"], priceBand: "¥600–1000/晚 · 九月预算带，非实时价格", evidenceIds: ["xhs-search-wanning-holiday", "hotel-wanning-holiday", "map-hotel-wanning-holiday-inn"], verifiedAt: "2026-08-14" },
