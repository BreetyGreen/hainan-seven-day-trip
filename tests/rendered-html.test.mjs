...
test("server-renders the Hainan map-first trip", async () => {
  ...
  assert.match(html, /武汉 → 海口 → 万宁 → 陵水 → 三亚/);
  ...
  assert.match(html, /海口鸿园酒店公寓·雅诗阁臻选/);
  assert.match(html, /万宁日月湾中旅逐浪屿玥酒店/);
  assert.match(html, /海南清水湾金普顿酒店/);
  assert.match(html, /2025(?:<!-- -->)? 年开业/);
